...
import sys
N, M = map(int,sys.stdin.readline().split()) # 4 7
tree_list = list(map(int,sys.stdin.readline().split())) # 20 15 10 17
tree_list.sort() # 10 15 17 20

start, end = 1, max(tree_list) # 자를 나무 전체 길이 범위 1 ~ 20

while start <= end :
    mid = (start + end) // 2 # mid는 자를 나무 높이 10 - 11 - 12 - 13 - 14 - 15

    result = 0
    for i in tree_list :
        if i >= mid :
            result += i - mid # 22 -> 19 -> 16 -> 13 -> 10 -> 7

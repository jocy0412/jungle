...
for i in range(N):
    inputNum = int(sys.stdin.readline()) # 1, 5, 2, 10, -99, 7, 5

    if len(leftHeap) == len(rightHeap): # 최대 힙과 최소 힙의 크기가 같으면
        heapq.heappush(leftHeap, -inputNum)
    else: # 최대 힙이 최소 힙보다 크기가 크면
        heapq.heappush(rightHeap, inputNum)

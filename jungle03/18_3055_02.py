...
def bfs() :
    ...
    while queue :
        ...
        for i in range(4) :
            nx = x + dx[i]
            ny = y + dy[i]
            if matrix[x][y] == '*' : # 범위를 벗어나지 않으면서 .에 접근하지 않았거나 돌과 도착점에 가지 않는 경우
                if 0 <= nx < R and 0 <= ny < C and matrix[nx][ny] == '.' and matrix[nx][ny] != 'X' and matrix[nx][ny] != 'D' :
                    matrix[nx][ny] = '*'
                    queue.append([nx, ny])

            elif matrix[x][y] == 'S' : # 범위를 벗어나지 않으면서 .에 접근하지 않았거나 돌과 물에 가지 않는 경우
                if 0 <= nx < R and 0 <= ny < C and matrix[nx][ny] == '.' and matrix[nx][ny] != 'X' and matrix[nx][ny] != '*' :
                    matrix[nx][ny] = 'S'
                    visit[nx][ny] = visit[x][y] + 1
                    queue.append([nx, ny])

                if matrix[nx][ny] == 'D':
                    return print(visit[nx][ny])

                elif 0 <= nx < R and 0 <= ny < C and matrix[nx][ny] == '*' :
                    return print('KAKTUS')
